...
/// </summary>
public sealed class JsObjectReferenceService : IJsObjectReferenceService
{
    /// <summary>
    ...
    public async Task InvokeVoidAsync<T>(IJSObjectReference? jsObjectReference, string method, params object?[]? args)
    {
        if (jsObjectReference is not null)
        {
            var type = typeof(T);
            await jsObjectReference.InvokeVoidAsync($"{type.Name}.{method}", args);
        }
    }
    ...
    public async Task<TValue?> InvokeAsync<T, TValue>(IJSObjectReference? jsObjectReference, string method, params object?[]? args)
    {
        if (jsObjectReference is not null)
        {
            var type = typeof(T);
            return await jsObjectReference.InvokeAsync<TValue>($"{type.Name}.{method}", args);
        }
