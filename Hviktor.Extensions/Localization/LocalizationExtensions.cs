...
public sealed class LocalizationExtensions(IJSRuntime jsRuntime) : ILocalizationService
{
    ...
    public async Task<string> GetLanguageAsync()
    {
        ...
        catch (Exception ex) when (ex is JSException or ArgumentException or InvalidOperationException)
        {
            // Fallback to current culture if JS interop is unavailable (prerendering) or errors occur
            return CultureInfo.CurrentCulture.Name;
        }
    }
}